...
public class AddShowProcessor {
	...
	try {
		...
		if (episodes == null) {
		    ...
		    TvShow tvShow = metaDataDao.getShowInformation(showId, "en");
	...
	episodes.stream().forEach(
		e -> {
		    ...
		    LOGGER.debug("Adding Episode status {} for episode S{}E{} and show {}", epStatus.getStatus(), epKey.getSeason(),
			    epKey.getNumber(), showConfig.getId());
