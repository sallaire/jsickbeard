...
public class RegexFilterConfiguration {
	...
	String pattern = name.replace(" ", EPISODE_SEP) + EPISODE_SEP + "S" + StringUtils.leftPad(season.toString(), 2, '0') + EPISODE_SEP + "E"
	...
	LOGGER.debug("Testing name against episode [S{}E{}] regex", season, episode);
	if (!matchRegexs(name, getEpisodeRegex(showName, season, episode))) {
	    return false;
	}
	LOGGER.debug("Testing name against quality [{}] regex", quality);
	if (!matchRegexs(name, getQualityRegex(quality))) {
	    return false;
	}
	LOGGER.debug("Testing name against lang [{}] regex", lang);
